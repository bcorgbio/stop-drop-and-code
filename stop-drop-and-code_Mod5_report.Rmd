...
```{r, echo=FALSE}
dat%>%
  ggplot()+geom_boxplot(aes(x=activity,y=degC))+facet_grid(.~who)+labs(caption="Figure 1. Boxplot of Axial Body Temperature Between Resting and Active States by Subject")+theme(plot.caption=element_text(hjust=0.5) )

#plotting the means +/- 1SD
mean_temp%>%
  ggplot(aes(x=activity,y=mean_degC))+geom_point(size=5, color="pink")+facet_grid(.~who)+geom_errorbar(aes(ymin=mean_degC-sd_degC,ymax=mean_degC+sd_degC),width=0.3)+labs(caption="Figure 2. Mean of Axial Body Temperature Between Resting and Active States by Subject including +/- 1 Standard Deviation")
```
